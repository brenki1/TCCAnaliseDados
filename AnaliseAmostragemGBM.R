...
dados <-read_csv("monteCarloA.csv", show_col_types = FALSE)
...
Acuracia <- ggplot(dados, aes(x = ite)) +
  ...
  labs(title = "Acurácia média amostragem GBM (1000 iterações)", 
...
Tempo <- ggplot(dados,aes(x= ite)) +
  ...
  labs(title="Tempo de execução médio amostragem GBM (1000 iterações)",
       subtitle = "Linha: média cumulativa     Pontos: média por iteração",
       x = "Iteração",
       y = "Tempo (s)")+
  theme_classic()
